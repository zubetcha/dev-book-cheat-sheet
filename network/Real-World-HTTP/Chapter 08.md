...
- 애플이 개발한 동영상 스트리밍 재생 방식으로, 아직 표준화는 되지 않음
- 모바일 사파리 및 크롬, 데스크탑의 사파리 및 엣지에서 지원하며 사실상 모바일에서는 표준으로 자리 잡음

HLS의 특징

- 실시간 중계 등의 스트리밍에 사용
- 회선 및 속도에 맞춰 적절한 해상도의 동영상 선택 가능
- 자막과 음성 전환 가능
- 동영상 포맷으로는 MPEG-TS 사용, MP4 지원
- 영상에 H.264, 음성에 AAC, MP3, AC-3 지원

### 8.5.1 HLS의 비디오 태그

### 8.5.2 마스터의 .m3u8 파일

- .m3m8 파일은 파일 정보를 담은 텍스트 파일로, UTF-8 형식으로 된 재생 목록(m3u)
- .m3m8 파일은 다른 .m3m8 파일을 참조할 수 있음
- .m3m8 파일에는 자주 사용되는 HLS 기능들이 나열되어 있음

> m3u  
> 하나 혹은 여러 개의 미디어 파일 경로를 텍스트로 기재한 파일로 인코딩을 UTF-8로 명시했을 때 m3m8 확장자 사용  
> 윈도우 플에이어, 아이튠즈 등 대부분의 프로그램이 지원하고 있지만 정식 사양은 없어 대응 방식이 각기 다름

### 8.5.3 자막 .m3u8 파일

- 만약 특정 단위로 파일을 자르면 루트 .m3m8 파일에 잘린 자막 파일을 묶는 .m3m8 파일이 링크함

### 8.5.4 동영상 파일

- 마스터 인덱스의 .m3u8가 참조하는 인덱스를 대체 인덱스라고 함
- 이 대체 인덱스에는 동영상 파일이 나열되어 있음
- 자막 파일처럼 세그먼트로 나뉘어져 있음
  - 일반 재생 파일: #EXT-X-PLAYLIST-TYPE:VOD
  - 생방송: #EXT-X-PLAYLIST-TYPE:EVENT
  - 생방송 후 다시 영상을 재생하는 경우: 끝에 #EXT-T-ENDLIST 추가
  - 생방송 후 다시 재생하지 않고 종료: 재생이 끝난 세그먼트의 동영상을 삭제하여 최신 동영상만 남겨둠
...
- HTTP 사용
- 특수 프로토콜용 전용 서버를 설치하고 통신할 포트를 열 필요가 없음
- 웹 서버 등을 사용할 수도 있도 CDN도 사용 가능
  - 서버에서 필요한 설정: MIME 타입, 캐시 유효 기간

단점

- 스트리밍이라고 하지만 사실상 점진적 다운로드 방식임
- 각 세그먼트별 다운로드가 끝나지 않으면 다음 세그먼트 영상의 재생 지연이 발생함
  - 최대 화질의 경우 (세그먼트 단위 시간 X 2 + 인코딩 시간)만큼 지연 발생
- 원활하게 지원되지 않는 환경이 많음 (ex. 데스크탑 크롬)
- 반년마다 내용 개정
- 동영상 포맷 고정
- 콘텐츠 보호 DRM 제약

> DRM,  
> Digital Rights Managament의 약자로, 디지털 콘텐츠를 안전하게 보호할 목적으로 암호화 기술을 이용하여 허가되지 않은 사용자로부터 콘텐츠의 저작권 관련 당사자의 권리 및 이익을 지속적으로 보호 및 관리하는 시스템

### 8.5.6 HLS 전후의 역사

- 브라우저의 스트리밍 기술이 생기기 전에도 전용 플레이어를 사용한 스트리밍이 이루어짐
- 브라우저를 이용한 스트리밍 시청은 실시간성을 강화한 프로토몰인 RTMP(Real Time Messaging Protocol
  )가 많이 사용되었음
- 애플이 HLS를 발표한 후, 각 업체에서 HTTP 점진적 다운로드 제공하기 시작
...
> DASH,  
> Dynamic Adaptive Streaming over HTTP의 약자로, HTTP를 사용해 동적으로 적절한 비트레이트로 스트리밍하는 방식

- HLS와 마찬가지로 HTTP를 통한 점진적 다운로드를 핵심으로 하는 스트리밍 방식
- MPEG-DASH는 HLS와 달리 표준이라는 차이가 있음
- 다만 목표하는 방향은 HLS와 거의 같으며 HLS를 대폭 확장한 방식

### 8.6.1 MPEG-DASH와 HLS 재생 방법의 차이

- 재생 방법에 차이가 있지만 사실상 현재에는 차이가 거의 없어졌다고 봐도 무방함

HLS

- 브라우저가 .m3u8 파일을 해석해서 재생하는 시스템
- 미디어 플에이어 프레임워크를 사용하면 브라우저 이외에서도 재생할 수 있음

MPEG-DASH

- 브라우저 외의 뷰어 지원
- 데이터 분석은 자바스크립트로
- 재생은 브라우저의 코덱을 자바스크립트에서 다루는 API, HTML5 Media Source Extensions 이용

### 8.6.2 Media Presentation Description 파일의 구조

- MPEG-DASH의 매니페스트 파일
- .mpd 확장자이며, XML를 사용해 표시
- XML의 XLink를 이용해 여러 파일로 나눌 수 있음
- 생방송의 경우 동적으로 파일이 갱신되어 클라이언트가 주기적으로 새로고침하여 최신 데이터를 가져옴

요약

- HLS, MPEG-DASH 모두 HTTP 기반
- AWS 엘라스틱 트랜스코더를 사용하면 MPEG-DASH와 HLS 변환 가능
